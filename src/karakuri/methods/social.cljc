(ns karakuri.methods.social
  "social.cljc — karakuri DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from karakuri's own
...
(defn draft-observation-post
  "A dry-run observation post over karakuri's own domain (Web-service-to-CLI (絡繰 = the mechanism that drives a manual service by command; the karakuri-kaizen sense — clever automation that removes…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "karakuri R0: live social posting is Council Lv6+ + operator + "
